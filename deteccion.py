...
while cap.isOpened():
    ...
    # Revisar qué objetos encontró el modelo en este cuadro de video
    for r in resultados:
        for caja in r.boxes:
            clase = int(caja.cls[0]) # ID del objeto detectado
            
            # En la base de datos COCO: 2 es 'car' (carro), 3 es 'motorcycle' (moto)
            if clase == 2:
                carro_detectado = True
            elif clase == 3:
                moto_detectada = True

    # SIMULADOR DE LEDs EN PANTALLA___
    
    # "LED" Rojo para el Carro (Brillante si lo detecta, oscuro si no)
    ...
    # Mostrar la ventana con el video y los LEDs
    cv2.imshow('Simulador YOLO - Presiona Q para salir', frame)

    # Esperar a que el usuario presione la tecla 'q' para cerrar
